...
export default function SHA() {
    ...
    const v = Constants.SHAVariants.includes(router.query.v)
        ? router.query.bits
    ...
    const [shaVariant, setShaVariant] = useState(v)
    ...
    const handleHashBtnClick = async (event) => {
        ...
        const request = `/api/sha?plaintext=${plaintext}&v=${shaVariant.replace('SHA-', '')}`
    ...
    const handleShaVersionChange = (event) => setShaVariant(event.target.value)
